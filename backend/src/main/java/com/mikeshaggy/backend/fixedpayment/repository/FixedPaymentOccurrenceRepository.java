package com.mikeshaggy.backend.fixedpayment.repository;
...
public interface FixedPaymentOccurrenceRepository extends JpaRepository<FixedPaymentOccurrence, Long> {
    ...
    @Query("SELECT o.dueDate FROM FixedPaymentOccurrence o WHERE o.fixedPayment.id = :fixedPaymentId AND o.dueDate BETWEEN :from AND :to")
    List<LocalDate> findDueDatesByFixedPaymentIdAndDueDateBetween(
    ...
    @Query("""
        ...
        LEFT JOIN FETCH o.transaction
        WHERE fp.id IN :fixedPaymentIds
        AND o.status = :status
