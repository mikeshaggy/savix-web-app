...
export default function FundDepositModal({ isOpen, onClose, onSuccess, fund }) {
  const t = useTranslations();
  const { wallets } = useWallets();
  const { lang } = useLanguage();

  const [formData, setFormData] = useState({ sourceWalletId: '', amount: '', date: '', notes: '' });
  ...
  const sourceWallet = useMemo(
    () => wallets.find(w => w.id === parseInt(formData.sourceWalletId)),
    [wallets, formData.sourceWalletId]
  );

  const parsedAmount = parseFloat(formData.amount) || 0;
  const walletBalanceAfter = sourceWallet ? (sourceWallet.balance ?? 0) - parsedAmount : null;
  const fundAmountAfter = parsedAmount > 0 ? (Number(fund?.currentAmount) || 0) + parsedAmount : null;
  const progressAfter = fund?.targetAmount > 0 && fundAmountAfter !== null
    ? Math.min((fundAmountAfter / Number(fund.targetAmount)) * 100, 100)
    : null;

  const insufficientWallet = walletBalanceAfter !== null && walletBalanceAfter < 0;

  const validate = () => {
    const errs = {};
    if (!formData.sourceWalletId) errs.sourceWalletId = t('funds.errors.sourceWalletRequired');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      await fundApi.deposit(fund.id, {
        sourceWalletId: parseInt(formData.sourceWalletId),
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-4 sm:px-7 py-5 sm:py-6 space-y-5 max-h-[calc(90vh-150px)] overflow-y-auto">
            ...
            {/* Source Wallet */}
            <div>
              <div className={LABEL}>{t('funds.depositSource')} <span className="text-purple-300">*</span></div>
              <select
                value={formData.sourceWalletId}
                onChange={e => handleChange('sourceWalletId', e.target.value)}
                className={`${SELECT_MD} w-full ${errors.sourceWalletId ? 'border-red-500' : 'border-white/[0.055]'}`}
              >
              ...
              {sourceWallet && (
                <p className="text-[12px] text-white/40 mt-1.5">
                  {t('funds.walletBalance')}: <span className="text-white/60 font-mono">{formatCurrency(sourceWallet.balance ?? 0, lang)}</span>
                </p>
              )}
              {errors.sourceWalletId && <p className="text-red-400 text-xs mt-1.5">{errors.sourceWalletId}</p>}
            </div>
            ...
            {/* Live preview */}
            {parsedAmount > 0 && (sourceWallet || fundAmountAfter !== null) && (
              <div className="p-4 bg-[#131325] border border-white/[0.055] rounded-2xl space-y-2">
                ...
                {sourceWallet && (
                  <div className="flex justify-between text-[13px]">
                    ...
                    <span className={`font-mono font-semibold ${walletBalanceAfter < 0 ? 'text-red-400' : 'text-white/80'}`}>
                      {formatCurrency(walletBalanceAfter, lang)}
                    </span>
                  </div>
                )}
                {fundAmountAfter !== null && (
                  <div className="flex justify-between text-[13px]">
                    <span className="text-white/40">{t('funds.fundAfter')}</span>
                    <span className="font-mono font-semibold text-emerald-400">
                      {formatCurrency(fundAmountAfter, lang)}
                      {progressAfter !== null && (
                ...
                {insufficientWallet && (
                  <div className="mt-2 text-[11px] text-red-400/80 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
                    {t('funds.insufficientWalletBalance')}
                  </div>
                )}
              </div>
            )}
            ...
            {/* Date */}
            <div>
              <div className={LABEL}>{t('funds.depositDate')} <span className="text-purple-300">*</span></div>
              <input
                type="date"
                value={formData.date}
            ...
            {/* Notes */}
            <div>
              <div className={LABEL}>{t('funds.depositNotes')}</div>
              <textarea
                value={formData.notes}
                onChange={e => handleChange('notes', e.target.value)}
                rows={2}
                placeholder={t('funds.depositNotesPh')}
          ...
          <div className="flex items-center justify-end gap-2.5 px-4 sm:px-7 py-[18px] border-t border-white/[0.055] bg-[rgba(6,6,15,0.4)]">
            ...
            <Button type="submit" variant="primary" size="md" disabled={submitting || insufficientWallet}>
              {submitting ? (
